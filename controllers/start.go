...
func Start(m *entities.Message) (err error) {
	err = msgutils.SendMessage(&m.Chat, fmt.Sprintf("Welcome %s! I'll help you to keep track on and manage your YouTube stuff. Lets get started:", m.From.FirstName))
	if err == nil {
		...
		err = msgutils.SendMessage(&m.Chat, menuText)
	}
	return err
}
